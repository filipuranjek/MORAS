...
Proof. 
unfold ALU. intros. simpl. reflexivity.
Qed.

Lemma ALU_Or (x y : list B) : 
  length x = length y -> ALU x y O I O I O I = OrL x y.
Proof. 
simpl. revert x y. induction x, y.
  - simpl. intros. reflexivity.
  - simpl. intros. reflexivity.
  - simpl. intros. reflexivity.
  - intros. simpl. destruct a, b.
...
Proof.
intros. destruct H. destruct H0. unfold ALU. simpl.
assert(H_NotL : forall (n : nat), NotL(repeat O n) = repeat I n).
{
intros. induction n0.
  - now simpl.
  - intros. simpl. f_equal. rewrite IHn0. reflexivity.
}
